    ...
    public static class CheckersBuilder{
        ...
        public CheckersBuilder Figure(String figureType, String figureColor,int x, int y){
            if (x > Board.MAX_INDEX || x<Board.MIN_INDEX ||
                ...
                throw  new IllegalStateException("x adn y should be in range between: " +
                        Board.MIN_INDEX + " to "+Board.MAX_INDEX);
            }
        ...
        public Checkers build(){
            if (playerOne.length() == 0){
                throw new IllegalStateException("There is no player one.");
            }
            if (playerTwo.length() == 0){
                throw new IllegalStateException("There is no player two.");
            }
